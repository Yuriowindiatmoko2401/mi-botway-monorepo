...
class ActionDaftarJadwal(Action):
    ...
    def run(
        ...
        message = "jadwal" + """\n"""
        mycursor.execute(
            "select * from jadwal_kuliah where konsentrasi='"
            + konsentrasi
            + "' or ks='"
            + konsentrasi
            + "'"
...
class ActionDaftarNilai(Action):
    ...
    def run(
        ...
        if nim is not None:
            mycursor.execute(
                "select mata_kuliah, SKS, nilai from nilai_mahasiswa where NIM='"
                + nim
                + "'"
            )
        else:
            mycursor.execute(
                "select mata_kuliah, SKS, nilai from nilai_mahasiswa where nama='"
                + nama
                + "'"
            )
...
class ActionJadwalSholat(Action):
    ...
    def run(
        ...
        mycursor.execute(
            "select id from js_kota where nama_kota like '%" + kawasan + "%'"
        )
        ...
        url = "https://api.banghasan.com/sholat/format/json/jadwal/kota/{kota_id}/tanggal/{date_now}".format(
            kota_id=results_id, date_now=datetime.strftime(datetime.now(), "%Y-%m-%d")
        )
...
class ActionPredCuaca(Action):
    ...
    def run(
        ...
        mycursor.execute(
            "select nama_kota from kota_indo where nama_kota like '%" + kawasan + "%'"
        )
        ...
        url = "http://api.openweathermap.org/data/2.5/weather?q={kawasan},id&APPID={app_id}&units=metric".format(
            kawasan=kawasan_req, app_id="9926258e5fbe65eb74b1eb220416b8d2"
        )
